...
public class AudioTabFragment extends Fragment implements AdapterView.OnItemSelectedListener {
    ...
    public void encodeAudio() {
        ...
        Log.d(TAG, String.format("Testing AUDIO encoding with '%s' codec", audioCodec));
    ...
    public void createAudioSample() {
        Log.d(TAG, "Creating AUDIO sample before the test.");
        ...
        Log.d(TAG, String.format("Sample file is created with '%s'.", ffmpegCommand));
        ...
        if (result == 0) {
            ...
            Log.d(TAG, String.format("Creating AUDIO sample failed with rc=%d.", result));
